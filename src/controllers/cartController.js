  ...
  try {
    ...
    if (!findUser)
      return res.status(404).send({
        status: false,
        message: "UserId or user does not exists please register",
      });
    ...
    if (productCheck.isDeleted === true)
      return res.status(404).send({
        status: false,
        message: `${data.productId} this product is deleted`,
      });

    // QUANTITY
    if (data.quantity || data.quantity == "") {
      if (!validators.isValidNum(data.quantity))
    ...
    if (data.cartId || data.cartId == "") {
      ...
      let cartCheck = await cartModel.findOne({
        _id: data.cartId,
        userId: userId,
      });
      if (!cartCheck)
        return res
          .status(404)
          .send({ status: false, message: "no cart found" });
    }
    ...
    if (findCart || data.cartId) {
      ...
      if (list.find((item) => item == data.productId)) {
        let updatedCart = await cartModel.findOneAndUpdate(
          { userId: userId, "items.productId": data.productId },
          {
            $inc: {
      ...
      const updateData = await cartModel.findOneAndUpdate(
        { userId: userId },
        {
          $inc: {
            totalPrice: createData.totalPrice,
            totalItems: createData.totalItems,
          },
          $push: { items: createData["items"] },
        },
        { new: true }
      );
      return res
...
const updateCart = async function (req, res) {
  try {
    ...
    if (!cartId)
      return res
        .status(400)
        .send({ status: false, message: "please mention cartID" });

    if (!mongoose.isValidObjectId(cartId))
      return res
        .status(400)
        .send({ status: false, message: "please mention valid cartID" });
    ...
    if (!cart) {
      return res
        .status(404)
        .send({ status: false, message: "No such cart found" });
    }

    if (cart.items.length == 0) {
      return res
        .status(400)
        .send({ status: false, message: "nothing to delete in item " });
    }
    ...
    if (!productId)
      return res
        .status(400)
        .send({ status: false, message: "please mention productID" });

    if (!mongoose.isValidObjectId(productId))
      return res
        .status(400)
        .send({ status: false, message: "please mention valid productID" });
    ...
    if (!product) {
      return res
        .status(404)
        .send({ status: false, message: "No such product found in cart " });
    }

    if (!(removeProduct == 1 || removeProduct == 0))
      return res.status(400).send({
        status: false,
        message: "please mention 1 or 0 only in remove product",
      });

    //****************** if remove product : 1 ***************/

    let cTotalItems, cTotalPrice, pro;
    ...
    if (removeProduct == 1) {
      ...
      for (let i = 0; i < pro.length; i++) {
        if (pro[i].productId == productId) {
          ...
          cTotalPrice = cart.totalPrice - product.price; // updated total price

          if (pro[i].quantity == 0) {
            pro.splice(i, 1);
            let ded = cart.totalItems - 1;
            cTotalItems = ded; // only  if item quantity will become zero, totalItems
          } // will -1
      ...
      let updated = await cartModel.findOneAndUpdate(
        { _id: cartId },
        { items: pro, totalPrice: cTotalPrice, totalItems: cTotalItems },
        { new: true }
      );

      return res
        .status(200)
        .send({ status: true, message: "update successfull", data: updated });
    }

    //************** if remove product : 0 ************** *******/

    if (removeProduct == 0) {
      pro = cart.items; // array of items

      for (let i = 0; i < pro.length; i++) {
        if (pro[i].productId == productId) {
          cTotalPrice = cart.totalPrice - product.price * pro[i].quantity; //deducting products price from total price

          cTotalItems = cart.totalItems - 1; // decreasing totalItems quantity by 1
...
const getCart = async function (req, res) {
  try {
    ...
    if (!findCart) {
      return res.status(404).send({
        status: false,
        message: `Cart doesn't exists by this ${userId} `,
      });
    }
...
const deleteCart = async function (req, res) {
  try {
    ...
    if (!mongoose.isValidObjectId(userId)) {
      return res
        .status(400)
        .send({ status: false, message: "invalid userId in param" });
    }
